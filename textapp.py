...
if not openai_api_key:
    st.error("OPENAI_API_KEY is missing. Add it to .streamlit/secrets.toml or to a .env file, then restart Streamlit.")
    st.stop()
...
def initialize_resources():
    ...
    with open(BASE_DIR / "cleaned_data.txt", 'rb') as f:
    ...
    vectorstore1 = Chroma(
        ...
        persist_directory=str(BASE_DIR / "cleaned_data")
    )
    vectorstore2 = Chroma(
        embedding_function=embed_model, 
        persist_directory=str(BASE_DIR / "mental_health")
    )
    vectorstore3 = Chroma(
        embedding_function=embed_model, 
        persist_directory=str(BASE_DIR / "econ")
    )
